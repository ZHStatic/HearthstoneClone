...
- [x] 阶段 3.5：快照模拟基础链路
  ...
  - [x] 真实评分 vs 快照评分验证入口
  - [x] 合法动作快照模拟后评分日志入口
- [x] 阶段 3.6：评分优先级动作选择收口
  - [x] `ActionSelector` 从规则优先级过渡到快照模拟评分优先级
  - [x] `AIActionSelection` 携带被选动作的模拟评分
  - [x] `AIController` 日志显示模拟评分和真实评分变化
  - [x] `SnapshotSimulator` 的 `EndTurn` 模拟补齐回合开始关键状态
  - [x] AI 只主动执行不降低评分的动作，否则结束回合
...
- 阶段 3.5 基础链路已完成：快照数据、真实动作到快照动作映射、单步快照模拟、真实/快照评分对比和模拟后评分日志均已写入并通过 Unity 编译检查。
- 阶段 3.6 已收口：AI 选择动作时优先使用快照模拟评分；日志会显示被选动作的模拟评分；`EndTurn` 快照会模拟对手回合开始的抽牌、法力刷新和随从攻击权限恢复；AI 当前只主动执行不降低评分的动作。
- UI 拆分和 UI 复用刷新暂时延后，等功能更完整后统一大改。
- 下一步：观察 AI 是否过于保守，再决定是调整 `Evaluator` 权重，还是加入“允许小亏换节奏”的阈值。

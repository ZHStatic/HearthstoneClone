...
```text
Unity Play 模式中验证 Enemy AI 的出牌和目标选择
```

准备：

```text
1. 选中场景里的 GameManager。
2. 勾选 Enable Enemy AI。
3. 勾选 Disable Deck Shuffle For Debug。
4. 勾选 Log AI Hand On Turn Start。
5. 根据要验证的场景配置 Enemy Deck Data。
6. 注意：当前抽牌规则是 List 末尾 = 牌库顶，所以 Inspector 列表最后一张会最先抽到。
```
...
```text
1. 进入 Play 模式。
2. 玩家回合直接结束回合，让 Enemy 行动。
3. Console 先看 AI 手牌日志：
   AI 手牌：1费 xxx | 4费 xxx，当前法力：4/4
4. 再看 AI 行动日志：
   AI 行动：... | 理由：... | 结果：...
```
...
```text
1. AI 不会主动把伤害法术打到自己英雄或自己随从。
2. 多张牌可打时，优先选择费用更高的牌。
3. 非击杀伤害法术有多个敌方目标时，优先打敌方英雄。
4. 能击杀敌方随从时，优先解场。
5. 多个解场目标都能击杀时，优先处理攻击力更高的随从。
6. 解场伤害溢出越少，优先级越高。
```
...
```text
这是阶段 3.3 的调试验证方式，不是成熟项目最终方案。
成熟项目通常会用自动化测试、固定随机种子、战斗回放或 AI 调试面板。
当前项目先用 Inspector 调试开关和 Console 日志，成本低，也便于看懂每一步。
```

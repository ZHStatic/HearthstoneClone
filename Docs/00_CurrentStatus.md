...
阶段 2.3 嘲讽代码已实现，等待 Unity Play 模式验证。

当前停靠点：

```text
阶段 2.3 代码已实现：第二个关键词“嘲讽”
```
...
| `GameManager.cs` | 当前阶段的对局流程调度，包含冲锋召唤处理和嘲讽攻击目标检查 |
...
- 在 Unity Editor 中给一张测试随从，例如“城墙守卫”，把 `Keywords` 增加为 `Taunt`。
- 进入 Play 模式后确认：对方有活着的嘲讽随从时，不能攻击英雄。
- 确认：对方有活着的嘲讽随从时，不能攻击非嘲讽随从。
- 确认：攻击嘲讽随从可以成功。
- 确认：火花这类法术仍然可以选择任意合法目标，不受嘲讽限制。
...
- 当前冲锋和嘲讽不需要事件系统，战吼、亡语、圣盾这类机制不要急着硬写。
...
```text
在 Unity Editor 中给“城墙守卫”配置 Taunt，测试嘲讽随从、非嘲讽随从和英雄三类攻击目标。
```

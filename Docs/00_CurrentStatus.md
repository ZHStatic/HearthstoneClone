...
阶段 2.9 战斗日志与代码整理已经收口；阶段 2.10 的核心结构优化已完成。阶段 3 已完成 AI 基础行动、第一版动作选择、策略验证入口、第一版评估函数、快照模拟基础链路、评分优先级动作选择收口、保守度调校入口、出随从模拟补强和亡语伤害模拟。UI 拆分和 UI 复用刷新暂时延后，等功能更完整后统一大改。

当前停靠点：

```text
阶段 3.9 已写入：Enemy AI 自动行动、评分函数、快照模拟、模拟评分日志、评分优先级动作选择、允许小亏换节奏、出随从关键词和战吼模拟、亡语伤害模拟
当前重点：观察 AI 是否能正确评估击杀亡语随从的代价和主动送掉己方亡语随从的收益
已完成：动作建模 -> 合法动作生成 -> 统一执行入口 -> AI 自动回合 -> 选择原因日志 -> 调试验证入口 -> 评估函数 -> 快照模拟 -> 评分优先级选择 -> 最小收益门槛 -> 允许小亏阈值 -> 出随从模拟补强 -> 亡语伤害模拟
暂缓：UI 拆分 -> UI 复用刷新
下一步：通过 Play 模式观察 AI 面对亡语随从时的选择，再决定扩展搜索深度或调整 Evaluator 权重
```
...
阶段 3.9：亡语伤害快照模拟已写入
下一步：观察 AI 面对亡语随从时的选择，再决定扩展搜索深度或调整 Evaluator 权重

...
| 卡牌 | 费用 | 攻击 | 生命 | 战吼 | 用途 |
|------|------|------|------|------|------|
| 火焰学徒 | 2 | 2 | 2 | 对敌方英雄造成 1 点伤害 | 测试最小战吼结算 |
...
Attack = 2
Health = 2
Battlecry Type = DealDamageToEnemyHero
Battlecry Damage = 1
```
...
11. 打出火焰学徒，展示战吼文字和敌方英雄立刻扣 1 点生命。
...
- `BattlecryType`、`CardData.BattlecryType`、`CardData.BattlecryDamage` 支持第一版战吼数据链路。
- `CardView` 和 `MinionView` 会把关键词显示为中文，例如 `Charge` 显示为“冲锋”，`Taunt` 显示为“嘲讽”。
- `CardView` 会显示战吼文字，例如“战吼：对敌方英雄造成 1 点伤害”。
- `Player`、`Board`、`GameManager` 拆分玩家资源、战场状态和对局流程。
- Core 层不依赖 UI 层。
- UI 点击只调用 `GameManager` 方法，不直接修改规则数据。
- 当前基础伤害法术、冲锋、嘲讽和第一个战吼先由 `GameManager` 直接结算，后续复杂法术和关键词会逐步抽到事件系统。
